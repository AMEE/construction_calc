...
The project requires an initial client and super admin before it is usable.  There is currently only one client supported but the application shows how to easily turn the application into this into a SaaS model.  To create these fire up the script/console and do the following:
    User.create(:login => "yourlogin", :email => "youremail", :name => "yourname", :password => "yourpassword", :password_confirmation => "yourpassword")
    User.first.register!
    User.first.activate!
